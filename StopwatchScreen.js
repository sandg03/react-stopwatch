...
export default function StopwatchScreen() {
  ...
  // Pulse animation for the timer dot
  ...
  const handleLap = () => {
    ...
    setLaps(prev => [{ id: Date.now(), time: lapTime, number: prev.length + 1 }, ...prev]);
  };

  const handleRemoveLap = (id) => {
    setLaps(prev => {
      const filtered = prev.filter(l => l.id !== id);
      // Re-number laps after removal
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Animated.View style={[styles.statusDot, { transform: [{ scale: pulseAnim }], backgroundColor: isRunning ? '#00F5C4' : elapsed > 0 ? '#F5A623' : '#333355' }]} />
        </View>
        ...
        <View style={styles.buttonsSection}>
          ...
          <View style={styles.buttonRow}>
            <TouchableOpacity
              style={[styles.btn, styles.btnStop, elapsed === 0 && styles.btnDisabled]}
              onPress={handleStop}
              disabled={elapsed === 0}
              activeOpacity={0.75}
            >
            ...
            <TouchableOpacity
              style={[styles.btn, styles.btnLap, elapsed === 0 && styles.btnDisabled]}
              onPress={handleLap}
              disabled={elapsed === 0}
              activeOpacity={0.75}
            >
              ...
              {laps.map((lap) => {
                ...
                return (
                  <View
                    ...
                    style={[
                      styles.lapRow,
                      isFastest && styles.lapFastest,
                      isSlowest && styles.lapSlowest,
                    ]}
                  >
                    <Text style={[styles.lapNumber, isFastest && styles.lapFastestText, isSlowest && styles.lapSlowestText]}>
                      #{String(lap.number).padStart(2, '0')}
                    </Text>
                    <Text style={[styles.lapTime, isFastest && styles.lapFastestText, isSlowest && styles.lapSlowestText]}>
                      {formatTime(lap.time)}
                    </Text>
                    {isFastest && <Text style={styles.lapBadge}>⚡ BEST</Text>}
                    {isSlowest && <Text style={[styles.lapBadge, styles.lapBadgeSlow]}>🐢 SLOW</Text>}
